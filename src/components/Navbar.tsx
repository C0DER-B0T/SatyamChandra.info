...
const Navbar = ({ activeTab, onTabChange }: NavbarProps) => {
  ...
  return (
    <nav className="fixed top-0 left-0 h-16 md:h-screen w-full md:w-20 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md shadow-lg z-50 transition-all duration-300">
      <div className="flex flex-row md:flex-col items-center justify-start md:py-8 h-full md:space-y-8 px-4 md:px-0 overflow-x-auto no-scrollbar scroll-smooth">
        {tabs.map((tab) => (
          <motion.button
            key={tab.id}
            onClick={() => onTabChange(tab.id)}
            className={`relative flex-shrink-0 p-2 md:p-3 mx-1 md:mx-0 rounded-lg transition-colors duration-300 ${
              activeTab === tab.id
